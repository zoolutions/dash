...
2. `git fetch origin <base>` then **`git merge origin/<base>`** — MERGE, never rebase. The branch is shared (it has a PR); a rebase would require a force-push, and `.claude/rules/git-workflow.md` forbids rebasing published branches. The base is normally `dash`; if the PR is a `feat/*` branch that's meant to be upstream-PR-able later, note in the report that merging `dash` in pollutes the upstream diff — the squash-merge upstreaming path in `.claude/rules/upstream-sync.md` hand-cleans that diff anyway.
