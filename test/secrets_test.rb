...
class SecretsTest < ActiveSupport::TestCase
  test "fetch" do
    with_test_secrets("secrets" => "SECRET=ABC") do
      assert_equal "ABC", Dash::Secrets.new(secrets_path: ".kamal/secrets")["SECRET"]
    end
  end

  test "synchronized_fetch" do
    with_test_secrets("secrets" => "SECRET=ABC") do
      assert_equal "ABC", Dash::Secrets.new(secrets_path: ".kamal/secrets").send(:synchronized_fetch, "SECRET")
    end
  end

  test "key?" do
    with_test_secrets("secrets" => "SECRET1=ABC") do
      assert Dash::Secrets.new(secrets_path: ".kamal/secrets").key?("SECRET1")
      assert_not Dash::Secrets.new(secrets_path: ".kamal/secrets").key?("SECRET2")
    end
  end
  ...
  test "variable references" do
    with_test_secrets("secrets" => "SECRET1=ABC\nSECRET2=${SECRET1}DEF") do
      assert_equal "ABC", Dash::Secrets.new(secrets_path: ".kamal/secrets")["SECRET1"]
      assert_equal "ABCDEF", Dash::Secrets.new(secrets_path: ".kamal/secrets")["SECRET2"]
    end
  end
  ...
  test "secrets file value overrides env" do
    with_test_secrets("secrets" => "SECRET1=DEF") do
      ENV["SECRET1"] = "ABC"
      assert_equal "DEF", Dash::Secrets.new(secrets_path: ".kamal/secrets")["SECRET1"]
    end
  end
  ...
  test "destinations" do
    with_test_secrets("secrets.dest" => "SECRET=DEF", "secrets" => "SECRET=ABC", "secrets-common" => "SECRET=GHI\nSECRET2=JKL") do
      assert_equal "ABC", Dash::Secrets.new(secrets_path: ".kamal/secrets")["SECRET"]
      assert_equal "DEF", Dash::Secrets.new(secrets_path: ".kamal/secrets", destination: "dest")["SECRET"]
      assert_equal "GHI", Dash::Secrets.new(secrets_path: ".kamal/secrets", destination: "nodest")["SECRET"]

      assert_equal "JKL", Dash::Secrets.new(secrets_path: ".kamal/secrets")["SECRET2"]
      assert_equal "JKL", Dash::Secrets.new(secrets_path: ".kamal/secrets", destination: "dest")["SECRET2"]
      assert_equal "JKL", Dash::Secrets.new(secrets_path: ".kamal/secrets", destination: "nodest")["SECRET2"]
    end
  end
  ...
  test "no secrets files" do
    with_test_secrets do
      error = assert_raises(Dash::ConfigurationError) do
        Dash::Secrets.new(secrets_path: ".kamal/secrets")["SECRET"]
      end
      assert_equal "Secret 'SECRET' not found, no secret files (.kamal/secrets-common, .kamal/secrets) provided", error.message

      error = assert_raises(Dash::ConfigurationError) do
        Dash::Secrets.new(secrets_path: ".kamal/secrets", destination: "dest")["SECRET"]
      end
