...
class GitTest < ActiveSupport::TestCase
  test "user name reads UTF-8 bytes under US-ASCII external encoding" do
    ...
    Dash::Git.expects(:`).with("git config user.name").returns(user_name)

    result = Dash::Git.user_name
  ...
  test "uncommitted changes exist" do
    Dash::Git.expects(:`).with("git status --porcelain").returns("M   file\n")
    assert_equal "M   file", Dash::Git.uncommitted_changes
  end

  test "uncommitted changes do not exist" do
    Dash::Git.expects(:`).with("git status --porcelain").returns("")
    assert_equal "", Dash::Git.uncommitted_changes
  end
end
