...
class ConfigurationAccessoryTest < ActiveSupport::TestCase
  ...
  test "env args" do
    with_test_secrets("secrets" => "MYSQL_ROOT_PASSWORD=secret123") do
      ...
      assert_equal [ "--env", "MYSQL_ROOT_HOST=\"%\"", "--env-file", ".dash/apps/app/env/accessories/mysql.env" ], config.accessory(:mysql).env_args.map(&:to_s)
      assert_equal "MYSQL_ROOT_PASSWORD=secret123\n", config.accessory(:mysql).secrets_io.string
      assert_equal [ "--env", "SOMETHING=\"else\"", "--env-file", ".dash/apps/app/env/accessories/redis.env" ], @config.accessory(:redis).env_args
